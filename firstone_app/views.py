...
def question_show(request, qid):
	if request.user is not None and request.user.is_active:
		...
		if Answer.objects.all():
			...
			return render(request, 'question_show.html', {'question_title':question.question_title, 'question_text':'期待你的答案', 'ask_user_nickname':ask_user.first_name, 'answer_time':'', 'answer_user_nickname':''})
	else:
		return HttpResponseRedirect("/login/")
